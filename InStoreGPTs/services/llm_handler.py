...
class LLMHandler:
    ...
    async def _post_completion_request(
        ...
        **kwargs,
        ...
        """Posts a completion request to the OpenAI completions API."""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                return await LLMHandler._post_completion_request(
                    session, headers, messages, tools, **kwargs
                )
            ...
            if inner_error_type == "ResponsibleAIPolicyViolation":
                error_content = "Guardrails violation: Responsible AI policy violation."
            else:
                error_content = "An error occurred while processing the request."
            return {
                "role": "assistant",
